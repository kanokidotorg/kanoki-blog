...
> `pandas.cut`**(**_x_**,** _bins_**,** _right: bool = True_**,** _labels=None_**,** _retbins: bool = False_**,** _precision: int = 3_**,** _include\_lowest: bool = False_**,** _duplicates: str = 'raise'_**)**
...
df = pd.DataFrame({'Sales':arr},index=pd.date\_range(start='1/1/2018', periods=24,freq='MS'))
...
df\['Sales\_Bins'\]=pd.cut(df.Sales,bins = \[0,2500,5000,7500,10000\])
df.head()

![pandas cut and qcut](/images/2020/04/image-15.png)

We added the respective bin values for sales in a new column Sales\_Bins

Let's count that how many months out of 24 falls into each bins

df\['Sales\_Bins'\].value\_counts().sort\_index()

(0, 2500\]         2
(2500, 5000\]      8
(5000, 7500\]     12
(7500, 10000\]     2
Name: Sales\_Bins, dtype: int64

We can see that the most of sales happens between 5000 and 7500

## **Label the bins**

We should get the original dataframe back first. So we will drop the extra column Sales\_bins for labelling

df.drop(\['Sales\_Bins'\],inplace=True,axis=1)
...
Interval Index is constructed using [pandas.Interval\_Range](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.interval_range.html?highlight=interval_range#pandas.interval_range)

> pandas.interval\_range(start=None, end=None, periods=None, freq=None, name=None, closed='right')
...
df\['Sales\_Bins'\]=pd.cut(df.Sales,bins = pd.interval\_range(start=0, end= 10000, periods=4))
df.head()
...
You can control this while creating the **IntervalIndex** using **interval\_rang** Closed parameter which can take any of these values
...
Add a new parameter include\_lowest and set it to true and check the result

df\['Sales\_Bins'\]=pd.cut(df.Sales,bins = \[ 849, 2500, 5000, 7500, 10000\],include\_lowest=True)
df.head()
...
df\['Sales\_Bins'\] = pd.qcut(df.Sales,4)
df.Sales\_Bins.value\_counts().sort\_index()
...
df\['Sales\_Bins'\] = pd.qcut(df.Sales,q=8,labels=\['1st','2nd','3rd','4th','5th','6th','7th','8th'\])
df.head(10)
...
df\['Sales\_Bins'\],bins = pd.qcut(df.Sales,q=8,labels=\['1st','2nd','3rd','4th','5th','6th','7th','8th'\],retbins=True)
bins

array(\[ 849. , 1371.5 , 2851. , 3395.5 , 4065. , 4889.75, 6208. , 8109.75, 9860. \])

Hope by now you will have a better understanding how qcut works and how it is different from the cut function
...
- IntervalIndex is one of the parameters that will give the range of values and timestamps to generate equally sized bins using pandas Interval\_Range
- By default the lowest interval of the bin is not inclusive so you have to set the include\_lowest as True explicitly
